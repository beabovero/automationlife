...
// Maps directly to automation profile fields used by the desktop runner
export interface JobConfig {
  accounts_count: number
  desired_name: string      // → {{profile.desiredName}} → InputProfileName step
  birthday: string          // YYYY-MM-DD → {{profile.birthday}} → InputBirthdayDay/Month/Year
  gender: 'male' | 'female' // → profile.gender → GenderTap + WhoToMeetTap steps
  photos: string[]          // Supabase storage paths → downloaded & uploaded to Geelark phone
  // SMS (operator-configured): Germany / PVAPins / bumble10 product
  // Bio: auto-generated by Claude Haiku at checkpoint 6.10 (ProfileQuestion step)
}
