...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('accounts').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('status', 'active').gte('updated_at', today.toISOString()),
    supabase.from('accounts').select('*', { count: 'exact', head: true }).eq('user_id', user.id).eq('status', 'active').gte('updated_at', weekStart.toISOString()),
  ])

  // Fetch accounts for any active jobs
  const activeJobIds = (jobs ?? []).filter(j => j.status === 'processing' || j.status === 'queued').map(j => j.id)
  let activeAccounts: { id: string; job_id: string; status: string; failure_reason: string | null }[] = []
  if (activeJobIds.length > 0) {
    const { data } = await supabase
      .from('accounts')
      .select('id, job_id, status, failure_reason')
