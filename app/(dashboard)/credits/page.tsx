...
function TxBadge({ type }: { type: CreditTransaction['type'] }) {
  const map: Record<string, string> = {
    purchase:    'text-[#00e5c8] border-[rgba(0,229,200,0.3)] bg-[rgba(0,229,200,0.06)]',
    ...
    <div className="p-8">
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-3">
        <div className="glass-card rounded-xl p-6 sm:col-span-1">
          <div className="font-mono text-[10px] tracking-widest text-[rgba(0,229,200,0.5)] uppercase">Balance</div>
          <div className="mt-2 font-mono text-5xl font-black text-[#00e5c8] text-glow">
            {(settings?.credits ?? 0).toFixed(0)}
          </div>
          <div className="mt-1 font-mono text-xs text-[rgba(224,224,224,0.4)]">credits · 1 credit = $1</div>
        </div>
        <div className="glass-card rounded-xl p-6">
          <div className="font-mono text-[10px] tracking-widest text-[rgba(0,229,200,0.5)] uppercase">Total Accounts Created</div>
          <div className="mt-2 font-mono text-3xl font-black text-white">{settings?.total_accounts_created ?? 0}</div>
        </div>
        <div className="glass-card rounded-xl p-6">
          <div className="font-mono text-[10px] tracking-widest text-[rgba(0,229,200,0.5)] uppercase">Plan</div>
      ...
      {/* Top up */}
      <div className="mb-8 rounded-xl border border-[rgba(0,229,200,0.15)] bg-[rgba(0,229,200,0.03)] p-6">
        <h2 className="mb-2 font-mono text-sm font-bold text-[#00e5c8]">Add Credits</h2>
        <p className="mb-4 text-sm text-[rgba(224,224,224,0.6)]">
          Contact <span className="font-bold text-[#00e5c8]">@aidetectionkiller</span> on Telegram to purchase credits via crypto.
        ...
        <div className="grid gap-3 sm:grid-cols-4 font-mono text-xs">
          {[
            ...
            <div key={name} className="rounded border border-[rgba(0,229,200,0.1)] bg-[rgba(0,0,0,0.3)] p-3">
              <div className="font-bold text-white">{name}</div>
              <div className="mt-1 text-[#00e5c8]">{price}</div>
              <div className="text-[rgba(224,224,224,0.4)]">{qty}</div>
              <div className="text-[rgba(0,229,200,0.4)]">{note}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {txs.map((tx, i) => (
                <tr
                  key={tx.id}
                  className={`border-b border-[rgba(0,229,200,0.06)] ${i % 2 !== 0 ? 'bg-[rgba(0,0,0,0.2)]' : ''}`}
                >
                  ...
                  <td className="px-4 py-3 font-mono text-xs">
                    <span className={tx.amount > 0 ? 'text-[#00e5c8]' : 'text-red-400'}>
